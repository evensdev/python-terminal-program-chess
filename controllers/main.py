...
class MainController:
    ...
    def main(self):
        ...
        response = self.display.get_input(
            'Choisissez parmis les propositions '
            '(de 1 à 5) : ',
            "number")
        ...
        while response != len(menu):
            ...
            if response == 1:
                ...
                self.display.affiche("------------------------- ")
                response = self.display.get_input(
                    'Votre choix est incorrect, réessayez : ',
                    "number")
...
class PlayerController:
    ...
    def menu_players(self):
        ...
        response = self.display.get_input(
            ...
            'pour aller sur le menu de votre choix :  ',
            'number')
    ...
    def create_player(self):
        ...
        self.display.affiche("------------------------- ")
        self.display.affiche("***  ATTENTION... "
                             "VOUS NE POUVEZ PAS REVENIR EN ARRIÈRE "
                             "SI VOUS AVEZ FAIT UNE ERREUR ! ***")
        self.display.affiche("***  VEUILLEZ SUPPRIMER LE JOUEUR "
        ...
        prenom = input('Ajoutez le prénom du joueur : ')
        print("")
        ...
        date_naissance = input('Ajoutez la date de naissance '
                               'du joueur jj/mm/aaaa : ')
        print("")
        ...
        self.display.affiche("        ")
        self.display.affiche("**********************************")
        self.display.affiche(f'Le nouveau joueur {player.prenom} '
                             f'{player.nom} '
    ...
    def delete_player(self):
        ...
        if len(PLAYER_SUBSCRIBED) == 0:
            ...
            response = self.display.get_input('Choisissez le bon numéro '
                                              'pour supprimer le joueur '
                                              'de votre choix '
                                              'ou tapez "0" '
            ...
            if response > quantite:
                print("")
                print("Ce numéro de joueurs n'existe pas !  "
                      "Retour au menu.")
                print("")
                ...
                PLAYER_SUBSCRIBED.pop(response - 1)
                print("")
                print("Le joueur",
                      name_deleted[0][0],
                      name_deleted[0][1],
    ...
    def list_of_player(self):
        ...
        if len(PLAYER_SUBSCRIBED) == 0:
            ...
            print("Il n'y a pas de joueurs inscrits. "
                  "Veuillez en ajouter en créant des joueurs")
    ...
    def update_ranking_player(self):
        ...
        if len(PLAYER_SUBSCRIBED) == 0:
            print("")
            print("Il n'y a pas de joueurs inscrits. "
                  "Veuillez créer des joueurs "
                  "pour modifier leur classement")
        else:
            numero = 0

            print("")
            print("=============================================")
            print("Choisissez le bon index pour modifier "
                  "le classement du joueur de votre choix")
            print("=============================================")
            print("")

            for player in PLAYER_SUBSCRIBED:
                numero += 1
                print(numero, " . ", player[0],
                      "  classement : ",
                      player[-1])

            print("")
            response = input("Sélectionnez votre joueur "
                             "en choisissant le bon numéro d'index "
                             "sur la gauche ou tapez sur la touche '0' "
                             "pour revenir en arrière :   ")
            ...
            if response == 0:
                ...
                updating = input(
                    f"Chosissez le nouveau n° de classement du joueur "
                    f" {PLAYER_SUBSCRIBED[response - 1][0]} :  ")

                PLAYER_SUBSCRIBED[response - 1][-1] = int(updating)
                print(" ")
                print(" ")
                print(PLAYER_SUBSCRIBED[response - 1][0],
                      f" est maintenant classé(e) à la place n°"
                      f"{PLAYER_SUBSCRIBED[response - 1][-1]}")
...
class TournoisController:
        ...
        if len(PLAYER_SUBSCRIBED) < 8:
            ...
            if TOURNOIS_LIST == [] and LIST_ROUNDS == []:
                ...
                if LIST_ROUNDS == []:
                    ...
                    print(LIST_ROUNDS)
                    print("")
                    print("Vous n'avez pas terminé votre tournoi. "
                          "Veuillez terminer vos rounds "
                          "ou clotûrer le tournoi "
                          "si vous avez fait entré les résultats "
                          "de tous les matchs")
                    print("")
    ...
    def create_round(self):
        if TOURNOIS_LIST == []:
            print("")
            print("--------------------------------------------")
            print("IMPOSSIBLE DE CRÉER UN ROUND SANS AVOIR "
                  "CRÉÉ UN TOURNOI...")
    ...
    def update_result(self):
        ...
        if LIST_ROUNDS == []:
            print("")
            print(" ***  VOUS NE POUVEZ PAS METTRE À JOUR "
                  "LES RÉSULTATS SI VOUS N'AVEZ PAS "
                  "CRÉÉ UN TOURNOI *** ")
            print("")
            print("############################# ")
            ...
            for item in LIST_ROUNDS[-1][3]:
                ...
                response = self.display.get_input(
                    'Qui est le vainqueur du match ?'
                    ' Choisissez le chiffre 1, 2 ou 0 :   ',
                    'number')
    ...
    def update_ranking_player(self):
        ...
        if len(PLAYER_SUBSCRIBED) == 0:
            ...
            print("")
            response = input("Sélectionner votre joueur en choisissant "
                             "le bon numéro :   ")
            response = int(response)
            print("")

            def update_ranking(response):
                updating = input(f"Chosissez le nouveau n° de classement "
                                 f"du joueur  "
                                 f"{PLAYER_SUBSCRIBED[response - 1][0]} :  ")
    ...
    def list_of_tournament(self):
        ...
        if len(TOURNOIS_LIST) == 0:
            print("")
            print("Il n'y a pas de tournoi(s) dans la liste. "
                  "Veuillez créer un tournoi.")
    ...
    def close_tournament(self):
        ...
        if TOURNOIS_LIST == []:
            ...
            if TOURNOIS_LIST[-1][-2] == 4:
                if LIST_ROUNDS[-1][-1][-1][-1][-1] is not None:
                    ...
                    print('')
                    print("Vous n'avez pas encore entré les résultats "
                          "du dernier Round")
                    print("")
            else:
                print("Vous devez terminer tout vos Rounds avant de "
                      "clôturer")
...
class RapportController:
    ...
    def menu_rapports(self):
        ...
        self.display.affiche("      ")
        response = self.display.get_input(
            'Choisissez parmis les propositions (de 1 à 8) : ',
            "number")
    ...
    def order_actor_by_alphabet(self):
        ...
        if PLAYER_SUBSCRIBED == []:
            ...
            print("")
            print("===================================")
            print("Voici la liste de Acteurs "
    ...
    def report_order_by_alpabet_tournament(self):
        ...
        if TOURNOIS_LIST == []:
            ...
            print("=================================================")
            print(
                "Sélectionnez le Tournoi de votre choix ci-dessous ")
            print("=================================================")
            print("")
        ...
        def ordering_player_tournament(tournament):
            ...
            if TOURNOIS_LIST == []:
                ...
                print("===========================================")
                print(
                    f"Voici la liste de joueurs du tournoi *** "
                    f"{TOURNOIS_LIST[tournament][0]}"
            ...
            print("")
            response = input("Choisissez votre tournoi"
                             " en sélectionnant le bon numéro : ")
            print("")
    ...
    def report_order_by_ranking_tournament(self):
        ...
        if TOURNOIS_LIST == []:
            ...
            response = input("Choisissez votre tournoi "
                             "en sélectionnant le numéro : ")
            print("")

            def ordering_player_tournament_by_ranking(tournament):

                list_sorted = sorted(TOURNOIS_LIST[tournament][2],
                                     key=lambda TOURNOIS: TOURNOIS[-1])
                index_item = 0

                print(
                    f"Voici la liste de joueurs du tournoi ***"
                    f" {TOURNOIS_LIST[tournament][0]}"
                    f" *** par ordre de CLASSEMENT : ")
                print("")

                for item in list_sorted:
                    index_item += 1
                    print(f" {index_item} -- "
                          f"{item[0]} || classement n°{item[-1]}")
    ...
    def list_of_tournament(self):
        ...
        if TOURNOIS_LIST == []:
            ...
            if len(TOURNOIS_LIST) == 0:
                print("")
                print("Il n'y a pas de tournois dans la liste. "
                      "Veuillez créer un tournoi.")
    ...
    def rapport_list_round_tournament(self):
        ...
        if TOURNOIS_LIST == []:
            ...
            if int(response) > len(TOURNOIS_LIST):
                print("Ce n'est pas un bon numéro, "
                      "choisissez votre tournoi "
                      "en essayant une dernière fois")
                response = input(
                    "Choisissez votre tournoi "
                    "en sélectionnant le numéro : ")
    ...
    def rapport_list_match_tournament(self):
        ...
        if TOURNOIS_LIST == []:
            ...
            print("========================================")
            print(
                "Sélectionnez le Tournois de votre choix ci-dessous")
            print("========================================")
            print(" ")
            ...
            if int(response) > len(TOURNOIS_LIST):
                ...
                print(f"Voici la liste des matchs du Tournoi *** "
                      f"{TOURNOIS_LIST[response][0]} ***")
                print("      | |  ")
                print("      | |  ")
                print("       v  ")

                print("")
                print("Voici les matchs du ",
                      TOURNOIS_LIST[response][-1][0][0])
                print("")
                print("")
...
class SaveController:
    ...
    def menu_save(self):
        ...
        self.display.affiche(" ")
        response = self.display.get_input(
            'Choisissez parmis les propositions (de 1 à 3) : ',
            "number")
